...
function returnMessage(winner) {
  if (winner === 0) {
    ...
    return "Você joga melhor do que eu esperava. 🤨";
  } else if (winner === 2) {
    return "Parece que eu jogo melhor que você. 🙃";
  }
}
...
function resetMatch() {
  ...
  talkToPlayer(
    `Bom saber que gostou do jogo. Para começar escolha seu primeira jogada que eu vou escolher a minha.`
  );
  return 1;
}
...
function checkWinner() {
  if (playerPoints > 4) {
    showResults("Você ganhou", 0);
  } else if (pcPoints > 4) {
    showResults("Você perdeu", 1);
  }
  return true;
}
...
// ====Game Logic =====
talkToPlayer(
  `Bem-vindo ao Pedra, Papel & Tesoura! É muito bom ter você aqui. Para começar escolha sua primeira jogada que eu vou escolher a minha.`
);
